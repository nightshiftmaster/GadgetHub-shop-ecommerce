...
import { MyFormValues, MyFormTouched } from "@/types/types";
...
const Step1 = ({ props }: { props: Step1Props }) => {
  ...
  return (
    <>
      <div className="flex gap-5 justify-center font-light  md:w-2/3 w-full  h-[15vh] ">
      ...
      {(Object.keys(values) as Array<keyof typeof values>)
        ...
        .map((input, i) => {
          ...
          return (
            <div
              className="flex w-full md:w-2/3 flex-col gap-5 font-light"
              key={i}
            >
              <label>
      ...
      <div className="flex w-full md:w-2/3 flex-col gap-5 font-light">
        ...
        <select
          ...
          className={`p-4 rounded-md ${
            errors.country && touched.country ? "ring-1 ring-red-500" : "ring-1"
          }`}
      ...
      <div className="flex flex-col w-full md:w-2/3 gap-5 font-light">
      ...
      <button
        className="bg-fuchsia-400 text-white p-3 w-1/3"
        type="submit"
        onClick={() => {
          Object.values(values)
            .slice(0, 7)
            .every((value) => value.length !== 0) && nextStep();
