...
const ProductsDisplay:React.FC<ProductsDisplayProps> = ({
    ...
    return (
        <div className={`w-full flex justify-center items-center`} data-testid={dataTestId}>
            <div className={`text-center flex flex-col w-[1250px] bg-lime-50 md:text-start pb-10 ${bgColor}`}>
                ...
                <div className="flex flex-wrap text-sm flex-1 gap-2 justify-center items-center">
                    {data?.slice(startIndex, endIndex).map((item: SingleProductType, i) => (
                        <Product key={i} {...item} />
                    ))}
                </div>
            </div>
